...
setup_killswitch() {
    ...
    iptables -A OUTPUT -d 192.168.0.0/16 -j ACCEPT      # allow local network
    iptables -A OUTPUT -d 10.0.0.0/8 -j ACCEPT          # allow local network
    iptables -A OUTPUT -d 172.16.0.0/12 -j ACCEPT       # allow local network
...
bring_up_wg() {
    ...
    # Add local network routes (bypass VPN for private subnets)
    if [ -n "$gw" ] && [ -n "$dev" ]; then
        ip -4 route add 192.168.0.0/16 via "$gw" dev "$dev" 2>/dev/null || true
        ip -4 route add 10.0.0.0/8 via "$gw" dev "$dev" 2>/dev/null || true
        ip -4 route add 172.16.0.0/12 via "$gw" dev "$dev" 2>/dev/null || true
    fi
