...
from app.services.voting_round_service import VotingRoundService
...
def get_all_rounds(
    ...
    if not rounds:
        return JSONResponse(
            status_code=404,
            content="No results where available in the db",
        )
...
def get_round_by_id(
    ...
    if not round_:
        return JSONResponse(
            status_code=404,
            content="No votes not found with that ID",
        )
...
def get_result_for_vote(
    ...
    votes = voting_service.get_all_votes_for_round_by_vote_id(number)
    ...
    if partyAcronim:
        votes = [
            v for v in votes
            if v.politician_rel and v.politician_rel.party_rel and v.politician_rel.party_rel.Acronym == partyAcronim
        ]
...
def get_all_votes_for_round_by_id(
    ...
    voting_service = VotingService(db)
    votes = voting_service.get_all_votes_for_round_by_guid(roundId)
    return [VoteSchema.from_orm_model(v) for v in votes]
